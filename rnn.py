...
model = Sequential()
model.add(SimpleRNN(128, input_shape=(X_train.shape[1], 1)))  # RNN katmanı
...
"""Önce tümünü çalıştırın sonray_train = tf.convert_to_tensor(y_train, dtype=tf.float64)kodu yerleştirin. y_train = tf.convert_to_tensor(y_train, dtype=tf.float64) olduğunda otomatik olarak patlıyor. Önce run edip sonra kodu yapıştırın."""
